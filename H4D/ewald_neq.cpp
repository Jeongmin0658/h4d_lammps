...
EwaldNeq::EwaldNeq(LAMMPS *lmp) : KSpace(lmp),
...
void EwaldNeq::settings(int narg, char **arg)
{
//  if (narg != 1) error->all(FLERR,"Illegal kspace_style ewald command - NONEQ EWALD");
//  accuracy_relative = fabs(utils::numeric(FLERR,arg[0],false,lmp));
  if (narg < 8 || narg > 11) error->all(FLERR,"Illegal kspace_style NONEQ EWALD command:::should be nine variables\naccuracy,time_origin_neq,time_diff_neq,max_altitude_neq,add_type_neq,isNeqinsertion,isNeqconvex,power_neq");
...
void EwaldNeq::init()
{
  if (comm->me == 0) utils::logmesg(lmp,"NonEQ 4D Ewald initialization ...\n");
...
double EwaldNeq::rms(int km, double prd, bigint natoms, double q2)
{
...
void EwaldNeq::vertical_screening()
{
  // Begin: NONEQ ALTITUE ============================================================================
  ...
  //std::string mesga = fmt::format("*****************************\nNEMD ewald altitude {}, {} = {:.8g},{:.8g},{:.8g} at {}/{}/{}\n",isNeqinsertion,delta,altitude,altitude2,altitude_screen,update->ntimestep,time_origin_neq,time_final_neq);
...
void EwaldNeq::compute(int eflag, int vflag)
{
  int i,j,k;
...
void EwaldNeq::eik_dot_r_triclinic()
{
  int i,k,l,m,n,ic;
  double cstr1,sstr1;
  double sqk,clpm,slpm;
...
void EwaldNeq::coeffs_triclinic()
{
  int k,l,m;
  double sqk,vterm;
...
void EwaldNeq::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void EwaldNeq::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
...
void EwaldNeq::deallocate_groups()
{
  // group A
