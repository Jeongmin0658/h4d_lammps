...
PPPMNeq::PPPMNeq(LAMMPS *lmp) : KSpace(lmp),
...
void PPPMNeq::settings(int narg, char **arg)
{
//  if (narg < 1) error->all(FLERR,"Illegal kspace_style pppm command");
//  accuracy_relative = fabs(utils::numeric(FLERR,arg[0],false,lmp));

  if (narg < 8 || narg > 11) error->all(FLERR,"Illegal kspace_style NONEQ PPPM command:::should be nine variables\naccuracy,time_origin_neq,time_diff_neq,max_altitude_neq,add_type_neq,isNeqinsertion,isNeqconvex,power_neq");
...
PPPMNeq::~PPPMNeq()
{
  ...
  delete [] factors;
  PPPMNeq::deallocate();
  if (peratom_allocate_flag) PPPMNeq::deallocate_peratom();
  if (group_allocate_flag) PPPMNeq::deallocate_groups();
...
void PPPMNeq::init()
{
  if (me == 0) utils::logmesg(lmp,"4D PPPM initialization ...\n");
...
void PPPMNeq::compute(int eflag, int vflag)
{
  int i,j;
  ...
  // JK: calculate mean altitude and screening factor that depends on the altitude
  vertical_screening();
  ...
  const double qscale = qqrd2e * scale * altitude_screen; // JK: screening
...
void PPPMNeq::deallocate_peratom()
{
  peratom_allocate_flag = 0;
...
double PPPMNeq::compute_df_kspace()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
double PPPMNeq::estimate_ik_error(double h, double prd, bigint natoms)
{
  double sum = 0.0;
...
double PPPMNeq::newton_raphson_f()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
void PPPMNeq::compute_gf_ik_triclinic()
{
  double snx,sny,snz;
  double argx,argy,argz,wx,wy,wz,sx,sy,sz,qx,qy,qz;
  double sum1,dot1,dot2;
...
void PPPMNeq::compute_sf_precoeff()
{
  int i,k,l,m,n;
...
void PPPMNeq::poisson_ik_triclinic()
{
  int i,j,k,n;
...
void PPPMNeq::vertical_screening()
{
  // Begin: NONEQ ALTITUE ============================================================================
...
void PPPMNeq::fieldforce_ik()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    const double qfactor = qqrd2e * scale * q[i] * altitude_screen; // JK: screening
    f[i][0] += qfactor*ekx;
    f[i][1] += qfactor*eky;
...
void PPPMNeq::fieldforce_ad()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    const double qfactor = qqrd2e * scale * altitude_screen; // JK: screening
...
void PPPMNeq::fieldforce_peratom()
{
  int i,l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void PPPMNeq::pack_forward_grid(int flag, void *vbuf, int nlist, int *list)
{
...
void PPPMNeq::unpack_forward_grid(int flag, void *vbuf, int nlist, int *list)
{
  FFT_SCALAR *buf = (FFT_SCALAR *) vbuf;
...
void PPPMNeq::pack_reverse_grid(int flag, void *vbuf, int nlist, int *list)
{
  FFT_SCALAR *buf = (FFT_SCALAR *) vbuf;
...
void PPPMNeq::unpack_reverse_grid(int flag, void *vbuf, int nlist, int *list)
{
  FFT_SCALAR *buf = (FFT_SCALAR *) vbuf;
...
void PPPMNeq::procs2grid2d(int nprocs, int nx, int ny, int *px, int *py)
{
...
void PPPMNeq::compute_rho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                         const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPMNeq::compute_drho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                          const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
void PPPMNeq::compute_rho_coeff()
{
  int j,k,l,m;
  FFT_SCALAR s;
...
int PPPMNeq::timing_1d(int n, double &time1d)
{
  double time1,time2;
...
int PPPMNeq::timing_3d(int n, double &time3d)
{
  double time1,time2;
...
void PPPMNeq::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void PPPMNeq::deallocate_groups()
{
  group_allocate_flag = 0;
...
void PPPMNeq::make_rho_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  int l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
...
void PPPMNeq::poisson_groups(int AA_flag)
{
  int i,j,k,n;
...
void PPPMNeq::poisson_groups_triclinic()
{
  int i,n;
...
void PPPMNeq::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
